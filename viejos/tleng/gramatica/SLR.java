...
public class SLR {
    ...
    Vector  _estados;
    /**
     * Es para la tabla, es un vector de hashtables.
     * Con  ht = _estados.get(5) se obtiene la hashtable asociada a la fila 6.
     * Y con  ht.get("a") se obtiene un vector con String con las producciones
     * correspondientes a ese lugar de la tabla
     */
    ...
    public SLR(GLC gram) throws Exception {
        System.out.println("Generando tabla de parsing...");
        _gram = gram;
        ...
        for (int i = 0 ; i < nodos.size() ; i++){           //para cada nodo
            ...
            for (int j = 0 ;  j < items.size(); j++){       //para cada ítem
                ...
                //hay que agregar 'Reduce'
                    ...
                    //Lleno en todas las columnas de los terminales 'ter' tal que
                    //'ter' pertenezca a siguientes(izq)
                    Vector sig = _gram.siguientes(izq);
                    for (int r = 0 ; r < _gram.getTerminales().size(); r++){
                        String ter = (String)_gram.getTerminales().get(r);
                        if (sig.contains(ter)){
                            //recupero el vector que tiene los red.|acc.|shift para
                            //agregarle el nuevo
                            Vector valor = (Vector) ht.get(ter);
    ...
    private void hacerAD(Item itemInicial) throws Exception{
        ...
        pila.push(new Tupla2(q0, 0)); //q0 = conj. de ítems

        while (!pila.empty()){//mientras haya cjto de ítems sin marcar
            Tupla2  t = (Tupla2)pila.pop();
            ...
            for (int i = 0; i < vPrima.size() ; i++){//para cada x de V'
                ...
                if (v2 != null && v2.size() > 0){
                    ...
                    if (z < 0){//=> v2 es un nuevo conjunto de ítems (nuevo nodo)
                        pila.push(new Tupla2(v2,idNodo));
        ...
        while (!pila.isEmpty()){
            ...
            if (item.hayMas()){
                ...
                if (_gram.esNoTerminal( simbolo )){
                    ...
                    if (x != null)
                        agregarSinRepetidos(pila, res, x);
                }
            }
        }
    ...
    /**
     * Transforma las producciones recibidas en ítems y si no están en res
     * las agrega en la pila.
     */
    private void agregarSinRepetidos(Stack pila, Vector res, Vector prod){
        for (int i = 0; i < prod.size(); i++ ){
            Produccion p = (Produccion)prod.get(i);

            Item niuItem = new Item(p);
