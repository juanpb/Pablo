...
//            fis = new FileInputStream(f);
            ...
            byte[] cont = new byte[tam];
            fis.read(cont);
            String x = new String(cont);
            int ls = x.indexOf(System.getProperty("line.separator"));
            String cab = x.substring(0, ls);
            String[] tits = parseCabezera(cab);
            x = x.substring(ls+1);
            ...
            while (st.hasMoreTokens()) {
                ...
                String tit = sacarComillas(tits[indCab]);
                if (indCab == 0){
                    us = new Element("Persona");
                    raiz.addContent(us);
                }
                indCab++;
                if (indCab == tits.length){
                    indCab = 0;
                }
                if (o.length() == 0)
                    continue;
