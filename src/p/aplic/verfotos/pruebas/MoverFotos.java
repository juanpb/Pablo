 ...
 * Time: 11:47
 */
public class MoverFotos {
    private static final Logger logger = Logger.getLogger(MoverFotos.class);

    private static final String dest = "D:\\P\\varios\\Fotos\\_tem\\May";

    public static void main(String[] args) throws Exception {
        new MoverFotos();
    }

    public MoverFotos() throws Exception {
        Config config = new Config("D:\\P\\_env\\config\\ver_fotos\\configMover.ini");
        config.setFiltroFotos("Mayra");  //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
//            System.out.println("f.getName() = " + f.getName());
    ...
    private List<File> getFotosDeTodosLosDirectorios(Config config)  {
        ...
        logger.info("Se cargarán fotos con las extensiones: " + config.getExtensiones());
        ...
        try {
            ...
            logger.info("Se mostrarán las fotos de los directorios: " + dirs + ", " + dirsSF +
                    ", que tienen " + res.size() + " fotos.");
